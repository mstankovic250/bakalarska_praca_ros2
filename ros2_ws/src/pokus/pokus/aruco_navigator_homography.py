...
class ArucoNavigator(Node):
    def __init__(self):
        ...
        self.camera_matrix = np.array([[497.087, 0, 316.839],
                                       [0, 497.601, 234.274],
                                       [0, 0, 1]])
        ...
        if ids is not None and len(ids) >= 4:
            ...
            if len(image_points) >= 4:
                ...
                if self.clicked_pixel is not None:
                    ...
                    transformed = cv2.perspectiveTransform(pixel, H)[0][0]

                    self.get_logger().info(
                        f"Kliknutý bod transformovaný do svetových súradníc (homografia): "
